...
for event in allEvents:
	...
	if event['entityManualList'] is not None:
		for manual in event['entityManualList']:
			...
			for automatic in event['entityAutomaticListLGAfterTraining']:
		...
		events.update({'_id':event['_id']},{"$set":{'manualAndAutomaticEqualitiesLGAfterTraining':entitiesWithEqualsValues}},upsert=False)
